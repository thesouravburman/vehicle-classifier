...
<div style="display:flex;align-items:center;justify-content:space-between;
    ...
    <div>
        <div style="font-family:'Montserrat',sans-serif;font-weight:800;
             font-size:1.2rem;color:#D4AF37;letter-spacing:0.08em;">SOURAV BURMAN</div>
        <div style="font-family:'Poppins',sans-serif;font-size:0.72rem;
             color:#EAD7A1;letter-spacing:0.15em;margin-top:2px;">CS ENGINEER · BUILDER</div>
    </div>
    <div style="font-family:'Montserrat',sans-serif;font-size:0.7rem;
         color:#D4AF37;opacity:0.5;letter-spacing:0.1em;">⬡ VEHICLE CLASSIFIER</div>
</div>
""", unsafe_allow_html=True)
...
Real-time vehicle detection from dash-cam footage · YOLOv8 · Samsung R&D Innovation Campus
</p>
""", unsafe_allow_html=True)

with st.sidebar:
    st.markdown("""<div style="font-family:'Montserrat',sans-serif;font-weight:800;
         color:#D4AF37;font-size:0.85rem;letter-spacing:0.1em;margin-bottom:1rem;
         padding-bottom:8px;border-bottom:1px solid #D4AF37;">SETTINGS</div>""",
         unsafe_allow_html=True)
    confidence = st.slider("Detection Confidence", 0.1, 0.9, 0.4, 0.05)

    st.markdown("""<div style="font-family:'Montserrat',sans-serif;font-weight:800;
         color:#D4AF37;font-size:0.85rem;letter-spacing:0.1em;margin:1.5rem 0 0.8rem;
         padding-bottom:8px;border-bottom:1px solid #D4AF37;">VEHICLE CLASSES</div>""",
         unsafe_allow_html=True)
    for name, color in [("Car","#1D4ED8"),("Motorcycle","#D4AF37"),
    ...
    st.markdown("""<div style="font-family:'Montserrat',sans-serif;font-weight:800;
         color:#D4AF37;font-size:0.85rem;letter-spacing:0.1em;margin:1.5rem 0 0.8rem;
         padding-bottom:8px;border-bottom:1px solid #D4AF37;">ABOUT</div>
    <div style="font-family:'Poppins',sans-serif;font-size:0.8rem;color:#EAD7A1;line-height:1.8;">
        ...
        Samsung R&D Innovation Campus<br/>Brainware University · CSE '27<br/><br/>
...
with tab1:
    uploaded = st.file_uploader("Upload a traffic image (JPG, PNG)", type=["jpg","jpeg","png"])
    ...
    if uploaded:
        ...
        with col1:
            st.markdown('<p style="color:#EAD7A1;font-family:Montserrat,sans-serif;font-weight:700;text-transform:uppercase;font-size:0.85rem;">Original Image</p>', unsafe_allow_html=True)
            st.image(image, use_column_width=True)
        with col2:
            st.markdown('<p style="color:#EAD7A1;font-family:Montserrat,sans-serif;font-weight:700;text-transform:uppercase;font-size:0.85rem;">Detected Vehicles</p>', unsafe_allow_html=True)
        ...
        if detections:
            ...
            with c1: st.metric("Total Vehicles", len(detections))
            ...
            with col_chart:
                ...
                fig.update_layout(
                    title=dict(text="VEHICLES BY TYPE",
                               font=dict(family="Montserrat",color="#D4AF37",size=13)),
                    paper_bgcolor="#0C0C0C", plot_bgcolor="#1F1F1F",
            ...
            with col_table:
                st.markdown('<p style="color:#EAD7A1;font-family:Montserrat,sans-serif;font-weight:700;text-transform:uppercase;font-size:0.85rem;">Detection Details</p>', unsafe_allow_html=True)
                df = pd.DataFrame([{
                    "Vehicle": d["label"],
        ...
        st.markdown("""
        <div style="border:2px dashed #D4AF37;border-radius:10px;padding:2.5rem;
             text-align:center;background:#1F1F1F;margin:1rem 0;">
            <div style="font-family:'Montserrat',sans-serif;font-weight:800;
                 color:#D4AF37;font-size:1rem;letter-spacing:0.08em;">
                UPLOAD A TRAFFIC IMAGE</div>
            <div style="color:#EAD7A1;font-size:0.85rem;margin-top:0.5rem;">
                Supports JPG and PNG · Max 200MB</div>
        </div>""", unsafe_allow_html=True)
...
with tab2:
    ...
    for num, title, desc in [
        ...
        ("02","PREPROCESSING","Image is converted and normalised for YOLOv8"),
...
with tab3:
    ...
    with col1:
        st.markdown("""<div class="result-card">
            <div style="font-family:'Montserrat',sans-serif;font-weight:800;
                 color:#D4AF37;font-size:0.8rem;letter-spacing:0.1em;margin-bottom:12px;">BACKGROUND</div>
            <div style="color:#EAD7A1;font-size:0.85rem;line-height:2;">
                ...
                Duration: Sept – Nov 2025<br/>Team: 4 members</div>
        </div>""", unsafe_allow_html=True)
    with col2:
        st.markdown("""<div class="result-card">
            <div style="font-family:'Montserrat',sans-serif;font-weight:800;
                 color:#D4AF37;font-size:0.8rem;letter-spacing:0.1em;margin-bottom:12px;">PERFORMANCE</div>
            <div style="color:#EAD7A1;font-size:0.85rem;line-height:2;">
                Model: YOLOv8n (Ultralytics)<br/>
                Accuracy: 90%+ mAP<br/>
                Speed: 30+ FPS on GPU<br/>Classes: 5 vehicle types</div>
        </div>""", unsafe_allow_html=True)
